            ...
            <!-- tabs区域 -->
                <el-tabs :tab-position="'left'"  v-model="activeIndex" :before-leave="beforeTabsLeave" @tab-click="tabClick">
                    ...
                    <el-tab-pane label="商品内容" name="4">
                        ...
                        <el-button type="primary" class="btnSty" @click="add()">添加商品</el-button>
                    </el-tab-pane>
                </el-tabs>
            </el-form>
        </el-card>
    ...
    methods: {
        ...
        async tabClick() {
            if (this.activeIndex === '1') {
                ...
                if (res.meta.status !== 200) {
                    return this.$message.error('获取属性参数列表失败！')
                }
        ...
        async add() {
            this.$refs.addFormRef.validate(async valid => {
                if (!valid) {
                    return this.$message.error('请正确填写必要的表单项！')
                }
                // 因为goods_cat与级联选择器进行双向绑定，所以不能用作修改
                // 深拷贝
